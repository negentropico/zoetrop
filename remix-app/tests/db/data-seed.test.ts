...
const SEEDED_TABLES = [
  ...
  { table: "protocol_changes", minRows: 20 },
  { table: "milestones", minRows: 8 },
...
describe.skipIf(!connectionString)(
  ...
  () => {
    for (const { table, minRows } of SEEDED_TABLES) {
      // Skip row-count assertions until Plan 03 seed script runs.
      // Un-skip this block after Plan 03 completes.
      it.skip(`${table} has >= ${minRows} rows with tenant_id set`, async () => {

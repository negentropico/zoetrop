...
- **Rehearsal + rollback path** — highest-risk migration rehearsed on a disposable Neon branch (2026-06-12) and rolled back to a fully clean state before the live apply (D-10). The VERIFIED rollback order is documented in the `0011_rls_policies.sql` header: (1) `DISABLE ROW LEVEL SECURITY` + `NO FORCE ROW LEVEL SECURITY` on all 16 tables, (2) `DROP POLICY` for every policy in schema public, (3) `REVOKE ALL ON ALL TABLES IN SCHEMA public FROM app_user`, (4) `REVOKE ALL ON ALL SEQUENCES IN SCHEMA public FROM app_user`, (5) `REVOKE USAGE ON SCHEMA public FROM app_user`, (6) `REVOKE ALL ON DATABASE neondb FROM app_user`, (7) `DROP ROLE IF EXISTS app_user`. Rehearsal findings: `DROP ROLE` fails unless ALL grants are revoked first; do NOT use `DROP OWNED BY app_user` (permission-denied on Neon as `neondb_owner`).

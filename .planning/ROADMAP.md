...
- [x] 07-07-PLAN.md — Wire Gate 3 (listAssignedSubjectIds -> assertSubjectAccess) into document.tsx PDF byte stream + consent.tsx loader/action (close CR-01 IDOR on raw lab PDFs); thread real actor role into insertAuthAuditLog (WR-04); extend wiring tests [AUTH-03]
- [x] 07-08-PLAN.md — Partial unique index `WHERE revoked_at IS NULL` on practitioner_subject_assignments (schema + migration 0014 + [BLOCKING] live Neon apply) so revoke-then-reassign works; structured 23505 error matching; assign->unassign->assign round-trip regression test (close CR-02) [AUTH-03]
...
| 7. PHI Compliance Hardening — RLS + Isolation Engineering | 8/8 | Complete   | 2026-06-12 |

...
- [x] **Phase 4: Static-to-DB Data Layer Migration** — Wire all route loaders to Neon via `withTenantDb`, seed owner's M0 data into live tables, remove PHI from TypeScript source, retire sync vestiges and `as any` casts (completed 2026-06-10 — 7/7 plans incl. gap-closure 04-06/04-07, verification passed 4/4)
...
- [x] 04-03-PLAN.md — Idempotent seed-data.ts orchestrator + run seed to Neon + DATA-02 row-count test green + capture parity fixtures from static modules (gitignored) [DATA-02]
...
- [x] 06-03-PLAN.md — Corpus authoring: LLM-assisted + human-reviewed owner-complete evidence-tiered corpus, seed to Neon (non-null K), retire genetics-knowledge.server.ts + re-point its 3 consumers [ENG-02, ENG-03]
...
**Wave 6** *(gap closure round 2 — close the two AUTH-03 blockers from re-verification 2026-06-12; depends on 07-01..07-06; the two plans touch disjoint files and run in parallel)*

- [ ] 07-07-PLAN.md — Wire Gate 3 (listAssignedSubjectIds -> assertSubjectAccess) into document.tsx PDF byte stream + consent.tsx loader/action (close CR-01 IDOR on raw lab PDFs); thread real actor role into insertAuthAuditLog (WR-04); extend wiring tests [AUTH-03]
- [ ] 07-08-PLAN.md — Partial unique index `WHERE revoked_at IS NULL` on practitioner_subject_assignments (schema + migration 0014 + [BLOCKING] live Neon apply) so revoke-then-reassign works; structured 23505 error matching; assign->unassign->assign round-trip regression test (close CR-02) [AUTH-03]
...
| 7. PHI Compliance Hardening — RLS + Isolation Engineering | 6/6 | Gaps Found (verification 2026-06-12) | - |
